  ...
  return (
    <section className="bg-white py-12 sm:py-16" aria-labelledby="featured-heading">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} className="grid min-h-[390px] touch-pan-y overflow-hidden rounded-lg border border-neutral-200 bg-neutral-950 shadow-xl lg:grid-cols-[1.35fr_1fr]">
          <AnimatePresence mode="wait">
            <motion.button key={'media-' + item._id} type="button" onClick={() => setPreview(item)}
              initial={desktopMotion ? { opacity: 0 } : false} animate={{ opacity: 1 }} exit={desktopMotion ? { opacity: 0 } : { opacity: 1 }} transition={{ duration: desktopMotion ? 0.45 : 0 }}
          ...
          <AnimatePresence mode="wait">
            <motion.article key={'text-' + item._id} initial={desktopMotion ? { opacity: 0, x: 20 } : false} animate={{ opacity: 1, x: 0 }} exit={desktopMotion ? { opacity: 0, x: -20 } : { opacity: 1, x: 0 }}
              transition={{ duration: desktopMotion ? 0.4 : 0 }} className="flex min-h-[260px] flex-col justify-center bg-white p-5 sm:p-7 lg:p-8">
        ...
        <div className="mt-5 flex items-center justify-center gap-2">
          ...
          {items.map((entry, itemIndex) => <button key={entry._id} type="button" onClick={() => setIndex(itemIndex)} aria-label={'Afficher ' + entry.title} className={'h-1.5 rounded-full transition-all ' + (itemIndex === index ? 'w-7 bg-emerald-700' : 'w-1.5 bg-neutral-300')} />)}

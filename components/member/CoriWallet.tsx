...
export function CauriBadge({ compact = false }: { compact?: boolean }) {
        ...
        <div className="fixed inset-0 z-[190] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div role="dialog" aria-modal="true" aria-labelledby="cauri-dialog-title"
            ...
            <div className="flex min-h-44 items-center justify-center bg-amber-50 p-6">
              <Image src="/images/cauris/cauri.png" width={170} height={170} alt="Cauri SALAM" className="h-auto w-full max-w-[170px] object-contain" />
            </div>
    ...
    <section className="rounded-lg border border-neutral-100 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-neutral-500">Gagnez des cauris en contribuant, puis utilisez-les lors des evenements SALAM.</p>
        </div>
        <CauriBadge />
      </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_130px_auto]">
        ...
        <input type="number" min={wallet?.redemption.minimum ?? 5} max={max} value={amount} onChange={event => setAmount(Number(event.target.value))} className="h-10 rounded-lg border border-neutral-200 px-3 text-sm outline-none focus:border-emerald-500" aria-label="Nombre de cauris" />

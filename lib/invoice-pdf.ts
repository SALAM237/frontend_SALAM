...
export function openInvoicePdfPreview(params: {
  ...
  const rows = params.lines.map(line => {
    ...
    return `
      <tr>
        <td class="line-cell">${rich(line.designation)}</td>
        <td class="right numeric-cell">${esc(line.qty)}</td>
        <td class="right numeric-cell">${fmtCfa(Number(line.ht || 0))}</td>
        <td class="right numeric-cell">${esc(line.vat)}%</td>
        <td class="right strong numeric-cell">${fmtCfa(ttc)}</td>
      </tr>
    `;
  }).join('');
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    ...
    table { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: clamp(9px, 1.35vw, 12px); table-layout: fixed; }
    ...
    .line-cell { max-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: clamp(7px, 1.05vw, 11px); line-height: 1.2; }
    .numeric-cell { white-space: nowrap; font-size: clamp(8px, 1.1vw, 12px); }
...
<body>
  <div class="page">
    ...
    <table>
      <colgroup><col style="width:46%" /><col style="width:11%" /><col style="width:16%" /><col style="width:11%" /><col style="width:16%" /></colgroup>
      <thead>
  ...
  const pages = documents.flatMap(doc => {
    ...
    return chunks.map((chunk, pageIndex) => {
      ...
      const rows = chunk.map(line => {
        const ttc = Number(line.qty || 0) * Number(line.ht || 0) * (1 + Number(line.vat || 0) / 100);
        return `<tr><td class="line-cell">${rich(line.designation)}</td><td class="right numeric-cell">${esc(line.qty)}</td><td class="right numeric-cell">${fmtCfa(Number(line.ht || 0))}</td><td class="right numeric-cell">${esc(line.vat)}%</td><td class="right strong numeric-cell">${fmtCfa(ttc)}</td></tr>`;
      }).join('');
      return `
        <div class="page-wrap">
        <article class="page">
          ...
          <section class="grid">
            ...
            <div class="card compact"><h2>Facturé à</h2><strong>${esc(doc.recipient.name)}</strong><p class="muted">${esc(doc.recipient.email)}</p><p class="muted">${esc(doc.recipient.phone)}</p><p class="muted">${esc(doc.recipient.address)}</p>${doc.recipient.memberId ? `<p class="muted">N° membre : ${esc(doc.recipient.memberId)}</p>` : ''}</div>
          </section>
          <table><colgroup><col style="width:46%" /><col style="width:11%" /><col style="width:16%" /><col style="width:11%" /><col style="width:16%" /></colgroup><thead><tr><th>Désignation</th><th class="right">Qté</th><th class="right">HT</th><th class="right">TVA</th><th class="right">TTC</th></tr></thead><tbody>${rows}</tbody></table>

...
import { useSendInternalMessage } from '@/lib/api/messages';
...
function ComposeModal({ onClose, initialRecipient }: { onClose: () => void; initialRecipient?: Recipient }) {
  ...
  const sendMessage = useSendInternalMessage('member');
  const [sent,      setSent]      = useState(false);

  const canSend = to.length > 0 && subject.trim() !== '' && body.trim() !== '';

  const handleSend = async () => {
    if (!canSend || sent || sendMessage.isPending) return;
    const recipientIds = [...to, ...cc, ...bcc].map(recipient => recipient.id);
    try {
      await sendMessage.mutateAsync({ recipientIds, subject: subject.trim(), content: body.trim() });
      setSent(true);
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center p-4 bg-black/50 backdrop-blur-sm sm:items-center">
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-neutral-200">
        ...
        <div className="flex gap-3 border-t border-neutral-100 px-5 py-4">
          ...
          <button
            onClick={handleSend}
            disabled={!canSend || sent || sendMessage.isPending}

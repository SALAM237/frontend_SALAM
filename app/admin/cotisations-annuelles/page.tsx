...
import {
  ...
  useSendCotisationAnnuelleReminders, useCotisationAnnuelleLogs,
  useResendCotisationAnnuelleReceipt,
  type CotisationAnnuelleStatus, type AdminCotisationAnnuelleRow,
...
function mapRows(data: AdminCotisationAnnuelleRow[], year: number): MemberRow[] {
  return data.map(r => ({
...
function openAnnuelleReceiptPdf(member: MemberRow, year: number) {
  const receiptNum = member.cotisationId
    ? `SALAM-CA-RECU-${year}-${member.cotisationId.slice(-6).toUpperCase()}`
    : `SALAM-CA-RECU-${year}-${member.userId.slice(-4).toUpperCase()}`;
  const invoiceNum = `SALAM-COT-${year}-${member.userId.slice(-4).toUpperCase()}`;
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td>Cotisation annuelle ${escReceipt(year)}</td>
...
function SettingsPanel({ year, deadline, setDeadline }: {
  ...
  const sendReminders = useSendCotisationAnnuelleReminders();
      ...
      <button onClick={() => setOpen(v => !v)}
        ...
        <div className="flex min-w-0 items-center gap-2.5 sm:gap-3">
          ...
          <div className="min-w-0 text-left">
            <p className="text-sm font-black text-neutral-900">Paramètres cotisation annuelle {year}</p>
...
function ReceiptModal({ member, year, onClose }: { member: MemberRow; year: number; onClose: () => void }) {
  const resendReceipt = useResendCotisationAnnuelleReceipt();
  const receiptNum = member.cotisationId
    ? `SALAM-CA-RECU-${year}-${member.cotisationId.slice(-6).toUpperCase()}`
    : `SALAM-CA-RECU-${year}-${member.userId.slice(-4).toUpperCase()}`;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-neutral-200">
        <div className="relative px-6 py-5" style={{ background: 'linear-gradient(135deg, #065f46 0%, #064e3b 60%, #022c22 100%)' }}>
          ...
          <p className="mt-1 text-lg font-black text-white">Reçu cotisation annuelle</p>
        ...
        <div className="border-t border-neutral-100 px-6 py-4">
          ...
          <button onClick={() => openAnnuelleReceiptPdf(member, year)}
...
/* ─── Page principale ─────────────────────────────────────── */
export default function CotisationsAnnuellesAdminPage() {
  const [year,       setYear]       = useState(new Date().getFullYear());
  const [deadline,   setDeadline]   = useState(`${new Date().getFullYear()}-06-30`);
  const [search,     setSearch]     = useState('');
  const [showLogs,   setShowLogs]   = useState(false);
  const [modal,      setModal]      = useState<MemberRow | null>(null);
  const [reminderSentId, setReminderSentId] = useState<string | null>(null);

  const { data: rawData, isLoading, isError } = useAdminCotisationsAnnuelles(year);
  const { data: logsData, isLoading: logsLoading } = useCotisationAnnuelleLogs();
  const updateStatus = useUpdateCotisationAnnuelleStatus();
  const deleteCotisation = useDeleteCotisationAnnuelle();
  ...
  const handleStatusChange = (member: MemberRow, newStatus: CotisationAnnuelleStatus) => {
  ...
  const confirmPayment = (data: { paidAt: string; reference: string; notes: string }) => {
    ...
    updateStatus.mutate(
      ...
      { onSuccess: () => { setModal(null); setTimeout(() => openAnnuelleReceiptPdf(paidMember, year), 250); } },
    );
  };

  const sendSingleReminder = useSendCotisationAnnuelleReminders();
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-xl font-black tracking-[-0.03em] text-neutral-900 sm:text-2xl">Cotisation annuelle</h1>
          <p className="mt-1 text-sm text-neutral-500">Gérer les cotisations annuelles des membres actifs.</p>
        </div>
        <div className="relative">
              ...
              return (
                <div key={member.userId} className="flex flex-wrap items-center gap-x-2 gap-y-2 px-3 py-3 transition-colors hover:bg-neutral-50/60 sm:flex-nowrap sm:gap-x-3 sm:px-5 sm:py-4">
                  ...
                  <div className="flex shrink-0 items-center gap-1 sm:gap-2">
                    ...
                    {member.status === 'paid' && (
                      <button onClick={() => openAnnuelleReceiptPdf(member, year)} title="Voir le reçu"
                    ...
                    <div className="relative">
                      <select value={member.status}
                        onChange={e => handleStatusChange(member, e.target.value as CotisationAnnuelleStatus)}
              ...
              return (
                <div key={log._id} className="flex items-start gap-4 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {fromLabel && toLabel && (
                      <p className="text-[11px] text-neutral-400 mt-0.5">{fromLabel} → {toLabel}</p>
                    )}
                  </div>
